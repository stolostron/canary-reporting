...
def populate_db(json_file):
    c.execute("CREATE TABLE IF NOT EXISTS snapshots (id text, time datetime, acm_release varchar(10), \
    ...
    c.execute("CREATE TABLE IF NOT EXISTS squad_tests (id text, time datetime, acm_release varchar(10), \
    ...
    if 'issue_url' not in d:
        if d['failed'] == 0:
            ...
            d['issue_url'] = 'https://github.com/ocmplus/backlog/issues'
    ...
    if c.fetchone() is None:
        add_tests_to_db(d, date_time_obj, release)
    conn.commit()

#function to read in json to database
def add_snapshot_to_db(json_data, date, release):
    c.execute("INSERT into snapshots values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                  [json_data['snapshot'], date, release, 
...
def add_tests_to_db(json_data, date, release):
    ...
    for row in squad_test_df.itertuples():
        c.execute("INSERT into squad_tests values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", 
                 [row[snapshot_idx+1], date, release, row[squad_idx+1], row[testsuite_idx+1], row[pass_idx+1], row[fail_idx+1], 
